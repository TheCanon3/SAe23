...
$capteur_selectionne = isset($_GET['capteurs']) ? $_GET['capteurs'] : '';
...
if ($capteur_selectionne) {
    ...
    $stmt_mesures = mysqli_prepare($conn, "
        SELECT mesures.valeur, mesures.date, capteurs.unite
        FROM mesures
        JOIN capteurs ON mesures.nom_capteur = capteurs.nom_capteur
        WHERE mesures.nom_capteur = ? AND mesures.date BETWEEN ? AND ?
        ORDER BY mesures.date DESC
    ");
...
$stmt_stats = mysqli_prepare($conn, "
    ...
    FROM salles
    JOIN capteurs ON salles.nom_salle = capteurq.nom_salle
    LEFT JOIN mesureq ON capteurs.nom_capteur = mesures.nom_capteur
    WHERE salles.id_batiment = ?
    AND mesures.date BETWEEN ? AND ? -- Filters for the last 24 hours.
    AND mesures.horaire BETWEEN ? AND ? -- Filters for the last 24 hours.
    ...
    <header>
        <h1>Panneau de Gestionnaire : <?php echo htmlspecialchars($nom_gestionnaire); ?></h1> <nav>
            <ul>
                ...
                <?php if (isset($_SESSION['role']) && $_SESSION['role'] === 'administration'): // Shows Admin link only for Admins. ?>
