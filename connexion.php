...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt_admin = mysqli_prepare($conn, "SELECT login, mdp FROM administration WHERE login = ?");
    ...
    if (mysqli_num_rows($result_admin) == 1) {
        ...
        if ($mot_de_passe === $admin['mdp']) {
            $_SESSION['user_id'] = $admin['login']; // Store user ID in session.
            $_SESSION['role'] = 'administration'; // Set user role.
