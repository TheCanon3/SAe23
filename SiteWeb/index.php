...
<body>
        ...
        <nav>
            <ul>
                ...
                <?php if (isset($_SESSION['role'])): // Check if a user is logged in. ?>
                    <?php if ($_SESSION['role'] === 'administration'): // If the role is Administrator, show the 'Administration' link. ?>
    ...
    <main>
        ...
        <section>
            ...
            $sql = "SELECT B.nom, S.nom_salle, S.type, S.capacite
                            FROM SAe23.batiments AS B
                            LEFT JOIN SAe23.salles AS S ON B.id_batiment = S.id_batiment
                            ORDER BY B.nom, S.nom_salle";
            ...
            if (mysqli_num_rows($result) > 0) {
                ...
                while($row = mysqli_fetch_assoc($result)) { // Loop through each row of data.
                    echo "<tr>";
                    echo "<td>" . htmlspecialchars($row['nom']) . "</td>"; // Display building name.
