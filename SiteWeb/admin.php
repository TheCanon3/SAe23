<?php
session_start();
// This checks if a user is logged in and if their role is 'administration'.
...
if (isset($_POST['ajouter_batiment'])) {
    $nom_batiment = $_POST['nom'];
    $login_gestionnaire = $_POST['login_gestionnaire'];
    $pswd_gestionnaire = $_POST['pswd_gestionnaire']; // IMPORTANT: Passwords should be hashed in production!

    // Prepare an SQL statement to insert a new building.
    $stmt = mysqli_prepare($conn, "INSERT INTO SAe23.batiments (nom, login_gestionnaire, pswd_gestionnaire) VALUES (?, ?, ?)");
    // Bind parameters to prevent SQL injection. 'sss' means three strings.
    mysqli_stmt_bind_param($stmt, "sss", $nom_batiment, $login_gestionnaire, $pswd_gestionnaire);
    ...
    $stmt = mysqli_prepare($conn, "DELETE FROM batiments WHERE id_batiment = ?");
...
if (isset($_POST['ajouter_salle'])) {
    ...
    $stmt = mysqli_prepare($conn, "INSERT INTO salles (nom_salle, capacite, type, id_batiment) VALUES (?, ?, ?, ?)");
    ...
    $stmt = mysqli_prepare($conn, "DELETE FROM salles WHERE nom_salle = ?");
...
$result_buildings = mysqli_query($conn, "SELECT id_batiment, nom FROM batiments");
$buildings = mysqli_fetch_all($result_buildings, MYSQLI_ASSOC);
mysqli_free_result($result_buildings); // Free memory.

$result_salles = mysqli_query($conn, "SELECT nom_salle FROM salles");
$salles = mysqli_fetch_all($result_salles, MYSQLI_ASSOC);
mysqli_free_result($result_salles);

$result_capteurs = mysqli_query($conn, "SELECT nom_capteur FROM capteurs");
    ...
    <main>
        <section>
            ...
            <form action="admin.php" method="POST">
                ...
                <label for="login_gestionnaire">Identifiant Gestionnaire :</label>
                <input type="text" id="login_gestionnaire" name="login_gestionnaire" required>
                <label for="pswd_gestionnaire">Mot de passe Gestionnaire :</label>
                <input type="password" id="pswd_gestionnaire" name="pswd_gestionnaire" required>
            ...
            <form action="admin.php" method="POST">
                ...
                <select name="id_batiment_delete" id="id_batiment_delete" required>
                    <?php foreach ($buildings as $b) { echo "<option value='{$b['id_batiment']}'>" . htmlspecialchars($b['nom']) . "</option>"; } ?>
                </select>
