...
$sql = "
    SELECT
        ...
        S.nom_salle,
        B.nom
    FROM SAe23.mesures AS M
    INNER JOIN SAe23.capteurs AS C ON M.nom_capteur = C.nom_capteur
    INNER JOIN SAe23.salles AS S ON C.nom_salle = S.nom_salle
    INNER JOIN SAe23.batiments AS B ON S.id_batiment = B.id_batiment
    WHERE M.id_mesure IN (
        SELECT MAX(id_mesure)
        FROM mesures
        GROUP BY nom_capteur
    )
    ORDER BY B.nom, S.nom_salle, C.nom_capteur;
";
        ...
        <nav>
            <ul>
                ...
                <?php if (isset($_SESSION['role'])): // Check if a user is logged in. ?>
                    <?php if ($_SESSION['role'] === 'administration'): // Show admin link if role is Administrator. ?>
